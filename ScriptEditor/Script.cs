...
namespace ScriptEditor
{
    class Script
    {
        ...
        private void onKeyUp(object sender, KeyEventArgs e)
        {
            if (ctrlPress)
            {
                if (!getRealStatus() && e.KeyCode == Keys.S)
                    save();
        ...
        private Boolean getRealStatus() { return (redoUndoPos == 0); }
        public void checkRedoUndoPos()
        {
            Boolean realStatus = getRealStatus();

            editor.save.Enabled = !realStatus;
            editor.saveAll.Enabled = !realStatus;
            modified = !realStatus;
